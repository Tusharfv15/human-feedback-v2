    ...
    def find_relevant_program(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.upper() == "NONE":
                ...
                print(f"[INFO] {self.agent_name}: No relevant program found, will use knowledge base")
                return {}
                ...
                print(f"[INFO] {self.agent_name}: Could not parse program selection '{response}', will use knowledge base")
                return {}
        
        except Exception as e:
            print(f"[ERROR] {self.agent_name}: Error in program search: {e}")
            print(f"[INFO] {self.agent_name}: Falling back to knowledge base due to error")
            self._clear_gpu_cache()
            return {}
...
- doping: Ion implantation, dopant activation, junction formation, annealing, boron/phosphorus/arsenic processes
- etching: Plasma etching, chemical etching, pattern transfer, material removal, selectivity
- wafer: Wafer cleaning, oxidation, CVD, packaging, surface preparation
...
You MUST follow the exact format above. Do not add any other text."""
            
            alpaca_prompt = self.create_alpaca_prompt(instruction, f"Query: {query}")
    ...
    def analyze_dependencies(self, solution: str, current_agent: str) -> tuple[bool, str, Optional[str]]:
        ...
        try:
            other_agents = [agent for agent in self.agents.keys() if agent != current_agent]
...
Current solution is from {current_agent} agent. Other available agents:
- doping: ion implantation, dopant activation, junction formation
- etching: plasma etching, pattern transfer, material removal
- wafer: cleaning, oxidation, CVD, surface preparation

Respond in EXACTLY this format:
DEPENDENCIES: YES or NO
EXPLANATION: [brief explanation]
NEXT_AGENT: [agent_name] (only if DEPENDENCIES is YES)

Examples:

Solution: "Use 50 keV boron implant after cleaning the wafer surface with RCA-1"
DEPENDENCIES: YES
EXPLANATION: Solution mentions wafer cleaning requirement
NEXT_AGENT: wafer

Solution: "Set plasma power to 500W for 2 minutes etching time"
DEPENDENCIES: NO
EXPLANATION: Solution is self-contained for etching process

You MUST follow the exact format above."""
            
            alpaca_prompt = self.create_alpaca_prompt(instruction, f"Solution to analyze:\n{solution}")
            ...
            for line in lines:
                ...
                if line.startswith("DEPENDENCIES:"):
                    ...
                    agent_part = line[11:].strip().lower()
                    if agent_part in self.agents and agent_part != current_agent:
                        next_agent = agent_part
            
            print(f"[DEBUG] Parsed - Dependencies: {has_deps}, Next: {next_agent}")
...
def dependency_analysis_node(state: MultiAgentState) -> Command[Literal["agent_processing", "human_feedback"]]:
    ...
    has_deps, explanation, next_agent = orchestrator.analyze_dependencies(solution, current_agent)
...
def continue_feedback_node(state: MultiAgentState) -> Command[Literal["dependency_analysis"]]:
    """Process feedback and continue with dependency analysis"""
    ...
    return Command(goto="dependency_analysis")
...
def finalize_node(state: MultiAgentState) -> MultiAgentState:
    ...
    print(f"Program Used: {state['selected_program'].get('task', 'Knowledge-based') if state['selected_program'] else 'Knowledge-based'}")
...
def create_multi_agent_workflow():
    ...
    builder.add_edge("continue_feedback", "dependency_analysis")
...
def run_multi_agent_workflow(user_query: str):
    ...
    while "__interrupt__" in result:
        ...
        print(f"Dependency Analysis: {interrupt_dict.get('dependency_analysis', 'N/A')}")
